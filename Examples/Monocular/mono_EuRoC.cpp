...
void LoadImages(const string &strPathimage,  const string &strPathTimes,
                vector<string> &vstrImage, vector<double> &vTimeStamps);
                ...
                vector<string> &vstrImage, vector<double> &vTimeStamps)
{
	...
	while (!fTimes.eof())
	{
		...
		if (!s.empty())
		{
			...
			ss >> t;
			ss >> sRGB;
			vstrImage.push_back(strPathimage + "/" + sRGB);
